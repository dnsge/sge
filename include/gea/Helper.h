...
/* The input file may be fed back in to replay your game session (autograder does this). */
    ...
    static inline unsigned int frame_number = 0;
    static inline Uint32 current_frame_start_timestamp = 0;
    static unsigned int GetFrameNumber() {
        return frame_number;
    }

    static SDL_Window* SDL_CreateWindow498(const char* title, int x, int y, int w, int h,
                                           Uint32 flags) {
        if (IsAutograderMode()) {
            x = 0;
            y = 0;
    ...
    static SDL_Renderer* SDL_CreateRenderer498(SDL_Window* window, int index, Uint32 flags) {
        if (IsAutograderMode())
            flags &=
                ~SDL_RENDERER_PRESENTVSYNC; // VSync is disabled to let frames render faster in the autograder.
    ...
    static void SDL_RenderPresent498(SDL_Renderer* renderer) {
        if (renderer == nullptr) {
            std::cout << "ERROR : The renderer pointer passed to Helper::SDL_RenderPresent498() is "
                         "a nullptr."
                      << std::endl;
            exit(0);
        }
        ...
        if (input_status == NOT_INITIALIZED) {
            std::cout << "ERROR : Please do not attempt to render (Helper::SDL_RenderPresent498()) "
                         "before you've entered the game loop (IE, begun calling "
                         "Helper::SDL_PollEvent498()."
                      << std::endl;
            exit(0);
        }
        ...
        if (RECORDING_MODE || _autograder_mode) {
            ...
            filenameStream << "frame_" << std::setw(5) << std::setfill('0') << frame_number
                           << ".bmp";
    ...
    static void CheckForRenderLoggerInit() {
        ...
        if (render_logger_mode == RL_NOT_INITIALIZED) {
            if (IsLoggingMode()) {
                ...
                render_logging_file << "Study the following SDL_RenderCopyEx498() calls to debug "
                                       "render-related issues."
                                    << std::endl;
                render_logging_file << "Enable render logger mode on your computer by setting the "
    ...
    static void SDL_RenderCopyEx498(int actor_id, const std::string &actor_name,
        ...
        if (render_logger_mode == RL_ENABLED) {
            ...
            render_logging_file << GetFrameNumber() << ":" << actor_id << ":" << actor_name
                                << " dstrect " << dstrect->x << " " << dstrect->y << " "
                                << dstrect->w << " " << dstrect->h << " angle " << angle
                                << " center " << center->x << " " << center->y << " flip " << flip
                                << " renderscale " << x_scale << " " << y_scale << std::endl;
        }
    }

private:
    static inline std::unordered_map<unsigned int, std::queue<SDL_Event>> frame_to_user_input;
    ...
    static void SDL_ConsiderInputFile() {
        ...
        if (RECORDING_MODE && input_status != INPUT_FILE_PRESENT && !_autograder_mode) {
            ...
            int num_events =
                SDL_PeepEvents(incoming_events, 100, SDL_PEEKEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT);
            ...
            for (int i = 0; i < num_events; i++) {
                ...
                if (event_type == SDL_KEYUP || event_type == SDL_KEYDOWN ||
                    event_type == SDL_MOUSEMOTION || event_type == SDL_MOUSEBUTTONDOWN ||
    ...
    static void SDL_Delay() {
        if (_autograder_mode) {
            ...
            int delay_ticks = std::max(static_cast<int>(desired_frame_duration_milliseconds) -
        ...
        while (std::getline(infile, line)) {
            ...
            while (std::getline(iss, eventStr, ';')) {
                ...
                event_type_string.erase(
                    std::remove(event_type_string.begin(), event_type_string.end(), '\r'),
