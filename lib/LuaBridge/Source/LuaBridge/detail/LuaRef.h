...
public:
    ...
    */
    ...
    //----------------------------------------------------------------------------
    /**
     * @brief Construct a new Lua Ref object, moving from an existing Lua Ref.
     *
     * @param other Existing Lua Ref.
     */
    LuaRef(LuaRef&& other) : LuaRefBase(other.m_L), m_ref(std::exchange(other.m_ref, LUA_REFNIL)) {}
    ...
    */
    ...
    //----------------------------------------------------------------------------
    /**
     * @brief Assign another LuaRef to this LuaRef, moving it.
     *
     * @param rhs A reference to assign-move from.
...
private:
    friend bool operator==(const LuaRef& lhs, const LuaRef& rhs);
...
inline bool operator==(const LuaRef& lhs, const LuaRef& rhs)
{
    LuaRef::StackPop p(lhs.m_L, 2);
    lhs.push();
    rhs.push();
    return lua_compare(lhs.m_L, -2, -1, LUA_OPEQ) == 1;
}
